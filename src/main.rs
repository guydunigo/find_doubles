...
use find_doubles::{async_version, find_doubles, multi_async, multithreaded, threaded, Comparison};
...
fn main() {
    ...
    if dir.is_dir() {
        ...
        } else if let Some("multi_async") = backend {
            multi_async::find_doubles(comp, &dir);
            println!("Multi-async finished");
